...
async function generateMarkers(device_id) {
    ...
    for (let i = 0; i < Points.length; i++) {
        var marker = InitMarker(
            +Points[i]["lat"],
            +Points[i]["lon"],
            i == 0 ? "user_marker" : "intrusion_marker"
        );
        Markers[i] = {
            marker: marker,
...
async function initMap(device_id) {
    ...
    map = tt.map({
        ...
        center: new tt.LngLat(
            Markers[0].marker.getLngLat().lng,
            Markers[0].marker.getLngLat().lat
        ),
    });
...
function create_device_card(device_id, device_ids) {
    ...
    card.addEventListener("click", function () {
        ...
        document.getElementById(`device_${device_id}`).className =
            "device_card_selected";
        initMap(device_id);
    });
    return card;
}
